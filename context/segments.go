...
const SegQueryClassification = `## 질문 유형 판별 (먼저 판별하고 해당 규칙을 따르세요)

### A. 매뉴얼/문법/개념/예제 질문 (TQL이 뭐야, SQL 사용법, 설정 방법, ~는 어떻게 해, ~예제 알려줘 등)
→ **당신의 사전 지식으로 답하지 마세요!** 반드시 문서를 검색한 후 답변하세요.
→ "~가 뭐야", "~란?", "~사용법", "~문법", "~예제", "~알려줘", "~설명해줘" 등 모든 질문/개념/예제 질문이 여기에 해당합니다.
...
const SegTableSchema = `## Machbase 테이블 구조
...
const SegAdvancedWorkflow = `## 고급 분석 (심층/다각도/FFT/RMS 키워드 포함 시)
...
const SegBasicWorkflow = `## 기본 분석 (분석해줘/대시보드 만들어줘)
...
const SegHTMLReportWorkflow = `## HTML 분석 리포트 ("리포트", "보고서" 키워드 포함 시)
→ 대시보드/TQL 파일을 만들지 마세요! save_html_report 도구의 설명에 있는 절차를 따르세요.
→ 도구 호출 시 모든 파라미터를 빠짐없이 전달하세요.
`

const SegErrorHandling = `## 에러 발생 시 (매우 중요!)
