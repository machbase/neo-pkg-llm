...
// Ollama-optimized compact segments (shorter than standard segments for smaller models)

const OllamaSegRole = `## 역할
...
const OllamaSegQueryClassification = `## 질문 유형 판별
...
const OllamaSegTableSchema = `## Machbase TAG 테이블
...
const OllamaSegAdvancedWorkflow = `## 고급 분석 절차 (TQL 차트만 사용)
...
6. save_tql_file → 해당 유형의 **모든 템플릿**을 TQL 파일로 저장!
   ...
   - **TAG, TAG1, TAG2는 반드시 2번에서 확인한 실제 태그명만 사용!**
   ...
   - 파일명은 영어만! (예: SILVER/avg_trend_1-1.tql)
   - UNIT: 수시간→'sec', 수일→'hour', 수주~수년→'day'
   - 금융: 1-1~1-6 (6개 전부) / 센서: 2-1~2-7 (7개 전부) / 범용: 3-1~3-4 (4개 전부)
   - 전부 저장할 때까지 7번으로 넘어가지 마세요!
...
const OllamaSegBasicWorkflow = `## 기본 분석 절차 (table-based 차트, TQL 불필요)
1. list_tables → 테이블 확인
...
const OllamaSegHTMLReportWorkflow = `## HTML 분석 리포트 (대시보드/TQL 만들지 마세요)
→ save_html_report 도구 설명의 절차를 따르세요. 모든 파라미터 필수! 리포트 URL 포함하여 보고.
`

const OllamaSegErrorHandling = `## 에러 대응
같은 에러 반복 시 다른 방법으로 전환. 1회 실패 후 문서 1회 참조.
`

const OllamaSegTQLRules = `## TQL 규칙
